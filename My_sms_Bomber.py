...
y='\033[1;33m'
w='\033[1;37m'
printLow(f'{r}▒█▀▀▀█ ░▀░ █▀▀█ ▒█▀▀█ █░░█ ░▀░ █▀▀ █░░█\n{w}░▀▀▀▄▄ ▀█▀ █▄▄▀ ▒█▄▄█ █▀▀█ ▀█▀ ▀▀█ █▀▀█\n{g}▒█▄▄▄█ ▀▀▀ ▀░▀▀ ▒█░░░ ▀░░▀ ▀▀▀ ▀▀▀ ▀░░▀\n\n{y}Info:\n    {g}[+] {y}My name: {w}Golden Power\n    {g}[+] {y}Channel: {w}Magic Editor\n   \n{y}system:\n    {g}[+] {y}Platform: {w}{System}\n    {g}[+] {y}Node: {w}{Node}\n    {g}[+] {y}Release: {w}{Release}\n\n' + "      " + g +'[+]' + y + "Time:" + w + hour + ":" + mi + ":" + ss + '\n\n')
def Vip(phone, Time):
...
while True:
    printLow(f'{g}[?] {y}Enter Phone Number {g}(+98) {r}- {w}')
    phone = is_phone(input("0"))
    
    if phone:
        break
try:
    printLow(f'{g}[?] {y}Enter Sleep Time Between Requests {g}[Defult=0] {r}- {w}')
    Time = float(input(""))
except ValueError:
    Time = 0
    print(f"{g}[0] {w}Used")
